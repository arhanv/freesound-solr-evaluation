...
selected_run = st.selectbox(
    "Choose the results folder to analyze (default should be most recent):", 
    options=run_folders, 
    index=0,
...
tab_summary, tab_visualize, tab_pygwalker = st.tabs([
    ":material/table_view: Summary Table",
    ":material/chart_data: Visualizations",
    ":material/explore: Explore Data (PyGWalker)"
], default=":material/chart_data: Visualizations")

with tab_summary:
    st.caption("Overview of key aggregated metrics for the evaluation run.")
    st.dataframe(results_df[["dims", "mean_latency", "recall_mean", "ndcg_mean", "qps", "space_size_mb"]])

# --- Visualizations for Aggregated/Global Metrics ---
with tab_visualize:
    if results_df is None or results_df.empty:
        st.error("No global results found.")
    else:
        with st.expander("Aggregated Metrics", expanded=True):
            g_tab0, g_tab1, g_tab2, g_tab3, g_tab4 = st.tabs(["Overview", "Recall@K", "Weighted nDCG@K", "Mean Latency", "QPS"])
            
            chart_df = results_df.sort_values('dims', ascending=True)
            ...
            with g_tab0:
                ...
                line_df = results_df.sort_values('dims', ascending=True)
                fig = make_subplots(rows=1, cols=4, subplot_titles=(
                    "Mean Latency", f"Recall@{metric_k}", f"nDCG@{metric_k}", "QPS"
                ))
                
                fig.add_trace(go.Scatter(x=line_df['dims'], y=line_df['mean_latency'], mode='lines+markers', line=dict(color='#2c3e50', width=3), marker=dict(size=8)), row=1, col=1)
                fig.add_trace(go.Scatter(x=line_df['dims'], y=line_df['recall_mean'], mode='lines+markers', line=dict(color='#e74c3c', width=3), marker=dict(size=8)), row=1, col=2)
                fig.add_trace(go.Scatter(x=line_df['dims'], y=line_df['ndcg_mean'], mode='lines+markers', line=dict(color='#27ae60', width=3), marker=dict(size=8)), row=1, col=3)
                fig.add_trace(go.Scatter(x=line_df['dims'], y=line_df['qps'], mode='lines+markers', line=dict(color='#3498db', width=3), marker=dict(size=8)), row=1, col=4)
                
                fig.update_layout(showlegend=False, height=350, margin=dict(l=20, r=20, t=40, b=20))
                for i in range(1, 5):
                    fig.update_xaxes(title_text="Dimensions", type="category", row=1, col=i)
                fig.update_yaxes(title_text="Latency (ms)", row=1, col=1)
                fig.update_yaxes(title_text="Recall", row=1, col=2)
                fig.update_yaxes(title_text="nDCG", row=1, col=3)
                fig.update_yaxes(title_text="QPS", row=1, col=4)
                
                st.plotly_chart(fig, width='stretch')
                st.caption(f"<div style='text-align: center'>Configurations - kNN(n={retrieve_n}) | {num_sounds:,} test queries | {index_size_str} | seed(s): [{seed}]</div>", unsafe_allow_html=True)
            
            with g_tab1:
                st.markdown(f"#### Mean Recall@{metric_k} vs Dimensions")
                fig = px.line(chart_df, x='dims_str', y='recall_mean', markers=True, color_discrete_sequence=['#e74c3c'],
                            labels={'dims_str': 'Dimensions', 'recall_mean': f'Mean Recall@{metric_k}'})
            ...
            with g_tab2:
                ...
                fig = px.line(chart_df, x='dims_str', y='ndcg_mean', markers=True, color_discrete_sequence=['#27ae60'],
                            labels={'dims_str': 'Dimensions', 'ndcg_mean': f'Mean nDCG@{metric_k}'})
                fig.update_layout(showlegend=False)
                st.plotly_chart(fig, width='stretch')
            ...
            with g_tab3:
                ...
                fig = px.line(chart_df, x='dims_str', y='mean_latency', markers=True, color_discrete_sequence=['#2c3e50'],
                            labels={'dims_str': 'Dimensions', 'mean_latency': 'Avg Latency (ms)'})
                fig.update_layout(showlegend=False)
                st.plotly_chart(fig, width='stretch')
            ...
            with g_tab4:
                ...
                fig = px.line(chart_df, x='dims_str', y='qps', markers=True, color_discrete_sequence=['#3498db'],
                            labels={'dims_str': 'Dimensions', 'qps': 'Queries Per Second'})
                fig.update_layout(showlegend=False)
                st.plotly_chart(fig, width='stretch')
    ...
    if details_df is not None and not details_df.empty and len(details_df) > 100:
        with st.expander("Query-by-Query Analysis", expanded=True):
            ...
            with q_tab1:
                view_type = st.segmented_control("Plot Style", ["Scatter Plot", "Line Plot", "Moving Average"], default="Line Plot")
                ...
                if view_type == "Scatter Plot":
                    ...
                    st.markdown("#### Moving Average of Query Latency `(window=50)`")
            ...
            with q_tab2:
                st.markdown("#### Latency Density Distribution")
...
with tab_pygwalker:
    if results_df is not None and not results_df.empty:
        ...
        if details_df is not None and not details_df.empty:
            pyg_data = st.segmented_control("Choose Dataset", ["Aggregated Metrics", "Query-by-Query Metrics"], default="Query-by-Query Metrics")
            
            if pyg_data == "Aggregated Metrics":
                pyg_df = results_df
        ...
        st.info("PyGWalker explorer is unavailable because no results were loaded.")
