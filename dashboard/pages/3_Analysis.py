...
st.set_page_config(page_title="Evaluation Analysis", layout="centered")
...
m1.metric("Seed", seed)
m2.metric("Num Queries", f"{num_sounds:,}")
m3.metric("Index Size", index_size_str)
st.divider()
...
if results_df is None or results_df.empty:
    ...
    st.subheader("Global Metrics Overview")
    ...
    with g_tab1:
        st.markdown("#### Mean Recall@K vs Dimensions")
        fig = px.bar(chart_df, y='dims_str', x='recall_mean', orientation='h', color='dims_str',
                     labels={'dims_str': 'Dimensions', 'recall_mean': 'Mean Recall@K'}, color_discrete_map=dim_to_color)
        fig.update_layout(showlegend=False)
        st.plotly_chart(fig, use_container_width=True)
        st.caption("**Recall@K**: Proportion of relevant items in top-K. Higher is better.")
        
    with g_tab2:
        st.markdown("#### Mean Weighted nDCG vs Dimensions")
        fig = px.bar(chart_df, y='dims_str', x='ndcg_mean', orientation='h', color='dims_str',
                     labels={'dims_str': 'Dimensions', 'ndcg_mean': 'Mean nDCG'}, color_discrete_map=dim_to_color)
        fig.update_layout(showlegend=False)
        st.plotly_chart(fig, use_container_width=True)
        st.caption("**Weighted nDCG**: Ranking quality (weighted top results). Higher is better.")
...
if details_df is not None and not details_df.empty and len(details_df) > 100:
    ...
    with q_tab3:
        warmup_cutoff = 0
        if results_df is not None and 'warmup' in results_df.columns:
            warmup_cutoff = int(results_df['warmup'].iloc[0])
        elif 'warmup' in config:
            warmup_cutoff = int(config['warmup'])
